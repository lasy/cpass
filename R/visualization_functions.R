...
#' @examples
#' data(PMDD_data)
#' input = PMDD_data %>% filter(SUBJECT == 2, CYCLE == 1)
...
plot_subject_obs = function(data = data.frame(), add_diagnosis = TRUE){
  ...
  plotlist = purrr::map(.x = cycles,
                        .f = function(cycle){
                          ...
                          plot_subject_cycle_obs(data = this_cycle_data, add_diagnosis = add_diagnosis)
                        })
...
plot_subject_data_and_diagnosis = function(data = data.frame(), save_as_pdf = TRUE, pdf_path = "", pdf_name = "", max_score_color = "tomato"){
