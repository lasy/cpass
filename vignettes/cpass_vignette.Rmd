...
```{r loading-devtools, warning=FALSE}
library(devtools)
```
...
```{r}
diagnosis$DRSP_level_diagnosis %>% filter(SUBJECT == 47, CYCLE == 2) %>% select(SUBJECT, CYCLE,DRSP, DRSP_meets_criteria, DSM5_SYMPTOM_DOMAIN) %>%  as.data.frame()
```
...
```{r plot-subject-diagnosis, fig.width=7, fig.height=5}
...
```{r plot-cycle, fig.height=6, fig.width=7}
...
```{r plot-cycle-3, fig.height=6, fig.width=7}
...
```{r plot-cycle-no-d, fig.height=6, fig.width=5}
...
```{r plot-cycle-color, fig.height=6, fig.width=7}
...
```{r plot-obs, fig.height=18, fig.width=7}
...
```{r plot-all, fig.height=25, fig.width=7}
